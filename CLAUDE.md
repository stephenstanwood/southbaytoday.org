...
The same 0a86b010 commit also unwired five section functions inside `TechnologyView.tsx`: `TechEventsSection`, `RecentlyFundedSection`, `SvHistorySection`, `AnnualConferencesSection`, `GovTechCallout`. They're still defined in the file but never called — do NOT re-call them, and do NOT extend the data that only they consume (`TECH_CONFERENCES`, `TECH_MILESTONES` window logic, etc.). The rendered Tech tab is: hero stats → Anchor Employers → `FundingHighlightsSection` (top 8 of `RECENTLY_FUNDED`) → Major Company Profiles → Hiring Pulse → `SpotlightHighlightsSection`. New funding entries DO render (via FundingHighlightsSection) — that's the right place for tech-coverage work.
