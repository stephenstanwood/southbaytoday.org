...
- House rules from the 2026-09 overhaul:
  ...
  - Views keep their first render free of clock, random, and localStorage reads (refine in `useEffect`) so hydration matches. When "today" has to be in the static HTML, read it through `useTodayPT(buildDayPt)` and count ages in Pacific calendar days (`calendarDaysAgo`). `SignalShell` hands the build's day to the tab islands that render it, so the page hydrates as built and then switches to the reader's day.
