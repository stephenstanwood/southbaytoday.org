...
Do NOT add heads-up banners, observance strips, school-calendar panels, holiday call-outs, or any other "above the filter bar" sections to the Events tab (`src/components/south-bay/views/EventsView.tsx`, between `sb-events-header` and `sb-events-sticky-filter`). Five banners were removed 2026-05-13 — SchoolHeadsUpBanner, SchoolYearEndgamePanel, HolidayHeadsUpBanner, HolidayPicksPreview, HeritageMonthBanner — because they cluttered the page without enough payoff (especially the school-dates content, which gets re-added on autopilot). The data files (`school-calendar.json`, `holidays.ts`, `heritageMonths.ts`) stay because they're used elsewhere — that's NOT a signal to re-wire a banner. WeekendPicksStrip is the only top-of-Events strip Stephen has approved. Routine work on filters, date pills, event cards, etc. is fine.
