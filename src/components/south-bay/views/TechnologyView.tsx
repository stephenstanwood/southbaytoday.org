...
// ISO date for N days ago in the user's local timezone — used to derive the
// "Last 30 Days" filter and the dynamic recent-rounds recap. Computed at
// module load (matches build-time freshness; the data file is regenerated by
// the same nightly job that triggers a redeploy).
...
const LAST_30_TOP_3 = [...LAST_30_ROUNDS]
  .filter((r) => parseFundingAmount(r.amount) !== null)
  .sort(
    (a, b) =>
      (parseFundingAmount(b.amount) ?? 0) - (parseFundingAmount(a.amount) ?? 0),
  )
  .slice(0, 3);
...
function RecentlyFundedSection() {
  const [stageFilter, setStageFilter] = useState<"all" | "early" | "last30">("all");
  ...
  const last30Count = LAST_30_ROUNDS.length;
  const filtered =
    stageFilter === "early"
      ? sorted.filter((r) => EARLY_STAGES.has(r.round))
      : stageFilter === "last30"
        ? sorted.filter((r) => r.date >= LAST_30_CUTOFF)
        : sorted;
  return (
    <div className="tech-section">
      <div className="tech-section-head">
      ...
      >
        ...
        >
          ...
          >
            {LAST_30_TOP_3.length > 0
              ? `Top rounds · last 30 days`
              : `Q1–Q2 2026 — South Bay VC surge`}
          </div>
          <div style={{ fontSize: 11, color: "var(--sb-muted)", lineHeight: 1.5 }}>
            {LAST_30_TOP_3.length > 0 ? (
              <>
                {LAST_30_TOP_3.map((r, i) => {
                  const last = i === LAST_30_TOP_3.length - 1;
                  const joiner =
                    i === 0 ? "" : last ? (LAST_30_TOP_3.length > 2 ? ", and " : " and ") : ", ";
                  return (
                    <span key={r.id + r.date}>
                      {joiner}
                      <strong style={{ color: "var(--sb-ink)", fontWeight: 600 }}>{r.name}</strong>
                      {" "}({r.amount}{r.round && r.round !== "Undisclosed" ? ` ${r.round}` : ""})
                    </span>
                  );
                })}
                {" "}led recent South Bay deal flow.
              </>
            ) : (
